...
Contact: tamby@tambako.ch https://www.shadertoy.com/view/MtVGDh
*/
...
const float specint = 0.01; // 0.2 // 0.01 // [c] test
const float specshin = 1.; // 20 // 1.

const float normdelta = 0.0004; // 0.0004
const float maxdist = 55.; // 55.
...
const float aawidth = 0.9; // 0.9
const int aasamples = 1;

// [c] test [[
vec4 iMouse = vec4(500.0, 500.0, 0.0, 0.0);
const vec3 iResolution = vec3(1080.0, 2342.0, 0.0);
// [c] test ]]
...
{
	vec3 rc = texture2D(uShadowTexture, ray.xy).rrr; //ray; //vec3(1.0, 0.0, 0.0); // texture(iChannel2, ray).rrr; // [c] test
    for (int l=0; l<3; l++)
...
RenderData trace0(vec3 tpos, vec3 ray, bool inside)
{
    ...
    {
        // Sky
        col = sky_color(ray); // vec3(0.0, 0.0, 0.0); // [c] test
        objnr = 3;
    }
